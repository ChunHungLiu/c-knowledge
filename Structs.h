...
public:
    ...
    inline Vector ( const Vector& v )					{ x = v[0]; y = v[1]; z = v[2];				}
    inline Vector ( float rgfl[3] )					{ x = rgfl[0]; y = rgfl[1]; z = rgfl[2];	}

    // Operators
    inline Vector operator- ( void ) const				{ return Vector ( -x, -y, -z );				}
    inline int operator== ( const Vector& v ) const	{ return x == v[0] && y == v[1] && z == v[2];	}
    inline int operator!= ( const Vector& v ) const	{ return ! ( *this == v );					}
    inline Vector operator+ ( const Vector& v ) const	{ return Vector ( x + v[0], y + v[1], z + v[2] );	}
    inline Vector operator- ( const Vector& v ) const	{ return Vector ( x - v[0], y - v[1], z - v[2] );	}
